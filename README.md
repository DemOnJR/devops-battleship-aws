...
- **Redis shared state** - Game state stored in Redis so both app instances share it. If one container restarts or fails, the other picks up active games with no data loss. Socket.IO Redis adapter syncs events across instances.
...
| Session store | Redis (shared game state + Socket.IO adapter) |
